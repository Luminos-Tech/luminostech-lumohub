 ...
 * >> 14 làm overflow / clipping nặng → tiếng rè, méo, không nghe được.
 */
...
esp_err_t mic_init(const mic_config_t *cfg)
{
     ...
     * Dùng STEREO khiến DMA đọc cả L+R, buffer bị interleave,
     * dễ gây alignment sai và lãng phí RAM gấp đôi.
     * MONO + SLOT_LEFT đọc thẳng chỉ kênh trái, sạch hơn.
     */
...
esp_err_t mic_read_frame(int16_t *buffer, size_t samples_capacity, size_t *samples_read)
{
    ...
    /**
     * FIX: MONO mode → mỗi sample thời gian chỉ có 1 slot 32-bit (không còn L+R pair).
     * Đọc thẳng s_frame_samples * sizeof(int32_t) bytes.
     */
