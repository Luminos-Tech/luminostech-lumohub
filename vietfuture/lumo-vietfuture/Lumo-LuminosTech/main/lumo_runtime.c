...
#define BEEP_WAV_PATH "/spiffs/beep.wav"
...
/* ========================================================================
 * LumoBand BLE alert system
 * BLE task writes sensor data → shared state.
 * Main loop reads & acts on alerts every ~200 ms (cooldown in lib).
 * ======================================================================== */

/* Audio feedback profiles per alert type.
 *
 * IMPORTANT: Some alerts (HIGH_ACTIVITY, HIGH/LOW HR, BATTERY) play
 * REPEATEDLY while the condition still holds. This function blocks
 * the calling task — it returns only when:
 *   - The condition has cleared, OR
 *   - The user acknowledged the alert (fall), OR
 *   - FEATURES.audio/storage is disabled.
 *
 * Other alerts (STEPS_GOAL, FALL_*) play once and return.
 */

/* Push alert task — runs in background, non-blocking */
...
static void band_alert_audio(band_alert_type_t alert, band_severity_t severity)
{
    if (!FEATURES.audio || !FEATURES.storage) {
        return;
    }

    /* --- Beep-loop alerts: keep playing while condition holds --- */
    switch (alert) {
    case BAND_ALERT_FALL_CONFIRMED:
        /* Tắt — chỉ giữ HIGH_ACTIVITY beep 5s */
        return;

    case BAND_ALERT_FALL_SUSPECT:
        /* Tắt — chỉ giữ HIGH_ACTIVITY beep 5s */
        return;

    case BAND_ALERT_HIGH_ACTIVITY:
        /* HIGH_ACTIVITY: peak >= 8g → phát risk.wav 2 lần + gửi push notification 3 lần */
        ESP_LOGW(TAG, "High-activity alert ON (peak=%.2fg)",
                 (double)g_band_state.peak_x100 / 100.0);

        // Phát 2 lần
        for (int i = 0; i < 2; i++) {
            esp_err_t ret = audio_play("/spiffs/risk.wav");
            if (ret == ESP_OK) {
                ESP_LOGW(TAG, "Risk sound %d/2", i + 1);
            } else {
                ESP_LOGE(TAG, "audio_play failed: %d", ret);
            }
        }
        ...
        ESP_LOGW(TAG, "High-activity alert done, back to main loop");
        return;
    ...
    default:
        break;  /* Fall through to one-shot alerts */
    }

    /* --- One-shot alerts (play once) --- */
    switch (alert) {
    case BAND_ALERT_STEPS_GOAL:
        /* Celebratory: goal reached! */
        audio_play("/spiffs/steps_goal.wav");
        break;

    case BAND_ALERT_SENSOR_OFFLINE:
        /* Soft single beep: wearable disconnected */
        audio_play("/spiffs/offline_alert.wav");
        break;

    default:
        /* BAND_ALERT_FALL_CONFIRMED + BAND_ALERT_HIGH_ACTIVITY +
         * BAND_ALERT_HIGH_HEART_RATE + BAND_ALERT_LOW_HEART_RATE +
         * BAND_ALERT_BATTERY_* → handled in beep-loop section above. */
        break;
    }
}

/* Print alert summary to serial log */
static void band_alert_log(band_alert_type_t alert, band_severity_t severity)
{
    const char *sev_str = (severity == BAND_SEVERITY_CRITICAL) ? "CRITICAL"
                         : (severity == BAND_SEVERITY_WARN)      ? "WARN"
                                                                : "INFO";
        ESP_LOGW(TAG, "BAND ALERT [%s]: %-22s | HR=%u peak=%.2fg steps=%u bat=%u%%",
             sev_str, band_alert_type_str(alert),
             g_band_state.heart_rate,
             (double)g_band_state.peak_x100 / 100.0,
             (unsigned)g_band_state.steps,
             (unsigned)g_band_state.battery);
}

/* Evaluate and act on band sensor alerts — called every loop iteration
 *
 * Returns:
 *   true  → alert action started (audio playing in another context or
 *           already finished one-shot)
 *   false → no alert to act on
 *
 * Note: HIGH_ACTIVITY, HR alerts, BATTERY and FALL_CONFIRMED are
 * *continuous* — they keep beeping in their own loop inside
 * band_alert_audio() until the condition clears. This function only
 * triggers the loop entry.
 */
static bool band_alert_tick(void)
{
    if (!FEATURES.network) {
        return false;
    }

    band_alert_type_t alert = band_evaluate_alerts();
    if (alert == BAND_ALERT_NONE) {
        return false;
    }

    band_severity_t severity = g_band_state.last_alert_sev;

    /* Suppress fall audio if user has acknowledged it.
     * Only acts when real_fall is still being detected; subject's
     * movement will clear it from band_in_danger_zone() automatically. */
    if (alert == BAND_ALERT_FALL_CONFIRMED && g_band_state.fall_alert_acknowledged) {
        ESP_LOGI(TAG, "Fall alert silenced (user acknowledged)");
        g_band_state.real_fall = false;
        g_band_state.in_danger_zone = false;
        return true;
    }

    /* Log once per tick so we don't spam serial while beeping */
    static band_alert_type_t s_last_logged = BAND_ALERT_NONE;
    if (s_last_logged != alert) {
        band_alert_log(alert, severity);
        s_last_logged = alert;
    }

    /* Continuous alerts: always enter the audio loop */
    if (alert == BAND_ALERT_FALL_CONFIRMED ||
        alert == BAND_ALERT_HIGH_ACTIVITY ||
        alert == BAND_ALERT_HIGH_HEART_RATE ||
        alert == BAND_ALERT_LOW_HEART_RATE ||
        alert == BAND_ALERT_BATTERY_CRITICAL ||
        alert == BAND_ALERT_BATTERY_LOW) {
        band_alert_audio(alert, severity);
        return true;
    }

    /* One-shot alerts respect cooldown */
    if (band_alert_can_fire(alert)) {
        band_alert_audio(alert, severity);
        g_band_state.last_alert_ms = esp_timer_get_time() / 1000ULL;
        return true;
    }
...
static void handle_button_press(bool first_event)
{
    ...
    if (g_band_state.fall_alert_active && !g_band_state.fall_alert_acknowledged) {
        ...
        band_acknowledge_fall();
        if (FEATURES.storage && FEATURES.audio) {
            audio_play("/spiffs/fall_cancelled.wav");
        }
    }
...
void lumo_runtime_start(const lumo_feature_config_t *features)
{
    ...
    for (;;)
    {
        ...
        // if (FEATURES.storage && FEATURES.audio)
        // {
        //     esp_err_t play_err = audio_play(BEEP_WAV_PATH);
        //     if (play_err != ESP_OK)
        //     {
        ...
        /* LumoBand BLE alert evaluation — checks thresholds, fires audio */
        band_alert_tick();
        audio_stop();
        vTaskDelay(pdMS_TO_TICKS(200));
    }
