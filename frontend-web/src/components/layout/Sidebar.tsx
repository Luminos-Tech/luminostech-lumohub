      ...
      <aside className={cn(
        ...
        {!isMobile && (
          <button
            ...
            className="absolute -right-3 top-8 flex items-center justify-center w-6 h-6 bg-white border border-gray-200 text-gray-500 rounded-full hover:bg-gray-50 hover:text-gray-900 shadow-sm z-50 transition-transform"
          >
        ...
        <div className={cn("mt-4 border-t border-gray-100 pt-4", collapsed ? "px-2 flex flex-col gap-2 items-center" : "")}>
          ...
          {collapsed && (
             <div className="w-10 h-10 rounded-full bg-primary-100 flex items-center justify-center text-primary-700 text-sm font-bold shrink-0 mb-2 mt-2" title={user?.full_name}>
               {user?.full_name?.charAt(0).toUpperCase()}
             </div>
          )}
          <button
            onClick={() => { logout().then(() => (window.location.href = "/login")); }}
            className={cn("flex items-center text-red-500 hover:bg-red-50 transition-colors", collapsed ? "p-3 rounded-lg justify-center w-full" : "gap-3 w-full px-3 py-2.5 rounded-lg text-sm")}
